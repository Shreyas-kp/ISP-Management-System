...
-- SELECT all records from the NetworkNodeSs Table
...
-- 1NF to 3NF 
-- We are moving from the original form (which can be considered a form of 1NF where AccountPlanDetails is a multi-valued attribute) to at least the 
-- Third Normal Form (3NF) by splitting the AccountPlanDetails into a separate table.
...
-- View to Display Customer Details with Addresses
CREATE VIEW CustomerDetails AS SELECT c.*, a.Street, a.City, a.Location, a.PinCode, a.Area FROM Customer c LEFT JOIN Address a ON c.CustomerID = a.CustomerID;
SELECT * FROM CustomerDetails;
-- DROP VIEW CustomerDetails;

-- View to Show Account Subscriptions
CREATE VIEW AccountSubscriptions AS
SELECT a.*, s.*
FROM Account a
LEFT JOIN Subscription s ON a.Account = s.AccountID;
SELECT * FROM AccountSubscriptions;
-- DROP VIEW AccountSubscriptions;

-- View to List Service Requests with Customer Information
CREATE VIEW ServiceRequestsView AS
SELECT sr.*, c.CustomerName, c.CustomerContact
FROM ServiceRequest sr
LEFT JOIN Customer c ON sr.CustomerID = c.CustomerID;
SELECT * FROM ServiceRequestsView;
-- DROP VIEW ServiceRequestsView;

-- View to Display Payments with Account Information
CREATE VIEW PaymentsView AS
SELECT p.*, a.AccountType, ap.PlanID, ap.PlanName, ap.PlanDescription
FROM Payment p
LEFT JOIN Account a ON p.AccountID = a.Account NATURAL JOIN AccountPlanDetails ap;
SELECT * FROM PaymentsView;
-- DROP VIEW PaymentsView;

-- View to Show Invoices with Customer Details
CREATE VIEW InvoiceDetails AS
SELECT i.*, c.CustomerName, c.CustomerAddress
FROM Invoice i
LEFT JOIN Account a ON i.AccountID = a.Account
